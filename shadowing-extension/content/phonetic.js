(function (root) {
  ...
  function phoneticSim(wa,wb){const ca=koelner(wa),cb=koelner(wb);if(!ca&&!cb)return 1;const d=levenshtein(ca,cb);return 1-d/Math.max(ca.length,cb.length,1);}
  ...
  function analyze(refText,hypText,opts){opts=opts||{};const refWords=sanitize(refText),hypWords=sanitize(hypText),pairs=alignWords(refWords,hypWords);
  ...
  const API={sanitize,koelner,levenshtein,phoneticSim,alignWords,classify,analyze};
